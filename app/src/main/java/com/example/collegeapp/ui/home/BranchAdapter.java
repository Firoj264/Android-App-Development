...
public class BranchAdapter extends PagerAdapter {
    ...
    public Object instantiateItem(@NonNull ViewGroup container, int position) {
        View view= LayoutInflater.from(context).inflate(R.layout.branch_item_layput,container,false);
        ImageView brimg;
        TextView brtitle,brdesription;
        brimg=view.findViewById(R.id.branch_icon);
        brtitle=view.findViewById(R.id.branch_title);
        brdesription=view.findViewById(R.id.branch_description);

        brimg.setImageResource(list.get(position).getImg());
        brtitle.setText(list.get(position).getTitle());
        brdesription.setText(list.get(position).getDescription());
